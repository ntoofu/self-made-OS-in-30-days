...
### Day6
* 参考
    - [割り込みについて](http://wiki.osdev.org/Interrupts)
    - [ISR, IRR](https://syuu1228.github.io/howto_implement_hypervisor/part4.html)
    - [cdeclの動作](http://vanya.jp.net/os/x86call/#cdecl)
...
* GDT(Global segment Descripter Table)
    ...
    - セグメントの管理用の属性では以下のものを管理
        + P: メモリ内に存在するか
        + DPL: 2bitでRing0-3の特権レベルを示す
        + S: システムセグメント or コードセグメント,データセグメント
        + Type:
            * データセグメント or コードセグメント
            * conforming(低い特権レベルからの実行)可否
            * セグメントがメモリアドレスのどちら方向に延びるか
            * 書き込み(データセグメントの時),実行(コードセグメントの時)の可否
            * 仮想記憶方式で利用するアクセスbit
#### 割り込み
* IDT(Interrupt Descriptor Table)
    - 割り込みに対し呼び出す処理の対応表
    - IDT自体はメモリ上に配置し、IDTの大きさと開始アドレスをIDTRレジスタ(48bit長)にLIDT命令で設定して使う
* IDTに基づいて呼び出される処理では、レジスタをスタックに退避したうえで処理を進め、終了後レジスタを復旧してIRETD命令によって復帰する
* PIC(Programmable Interrupt Controller)
    - 複数のハードウェアデバイスからの割り込み信号=IRQ(Interrupt ReQuest)を監視し、1つの出力でCPUに通知するコントローラ
    - PICは1つが8入力で2つをカスケードしてIRQ0-15(IRQ2はカスケード接続用)の15入力を扱い, それぞれにIRQ0-15まであるハードウェアからの割り込み要求番号を振っている
    - PIC自体がそれぞれIMR, ICW1-ICW4などのレジスタを持ち、通常OSはこれらをいじって割り込みの設定をする
        + IMR(Interrupt Mask Register): 初期化時や未使用時に割り込みを受付ないためのマスク設定
        + ICW(Initial Control Word): 割り込み番号の設定の他, 動作モードやマスタースレーブの接続ピン等の指定もする
    - 割り込みベクタ0-31はCPUからの例外処理用に予約されているが、real modeではIRQ0-7が08-0Fを使ったりする
    - PICはデバイスから通知された割り込み要求を, PICが内部に持つIMR, ISR, IRRレジスタを利用して通知すべき（優先度が高くマスクされていない）割り込みをCPUに通知する
* 割り込みコントローラは初期のPIC(i8259)以外にAPIC(Local APIC, I/O APIC)もあるし、MSIという異なる割り込みの仕組みもある
...
* 代表としてcdeclでは関数呼び出しは以下のように処理する
    1. (呼び出し元) push命令で引数を逆順にスタックに積んでいく
    2. (呼び出し元) call命令で戻りアドレスをスタックに積みつつジャンプ
    3. (呼び出し先) %ebp(frame pointer)をスタックに積んで退避
    4. (呼び出し先) %esp(stack pointer)を%ebpに退避(呼び出しによりフレームが移ったとも取れる)
    5. (呼び出し先) %ebpをベースに引数を処理する, ローカル変数は適宜スタックを伸ばして利用
    6. (呼び出し先) %ebpを%espにmovして%esp(stack pointer)復元(ローカル変数消去とも取れる)
    7. (呼び出し先) pop命令で%ebpをスタックからとってきて復元
    8. (呼び出し先) ret命令でスタックトップの戻りアドレスをポップしつつジャンプ
    9. (呼び出し元) %espを動かすなどして引数分のスタックを廃棄
